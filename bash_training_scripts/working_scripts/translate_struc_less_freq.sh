...
for model_checkpoint in $MODEL_DIR/$MODEL$STEP.pt
    do
        ...
        python $OPENNMT/translate.py -model $model_checkpoint -src $BASE/testset-struc-less-freq-source.tok -gpu $GPU -beam_size 30 -batch_size 1 -max_length 500 -output $OUTPUT -n_best 1
